...
export function ConversationPillars() {
  ...
  return (
    <section id="conversations" aria-labelledby="conversations-heading" className="section-pad border-t border-white/10 bg-black font-sans text-white">
      <div className="page-shell">
        <div className="grid gap-8 lg:grid-cols-[1fr_.65fr] lg:items-end">
          ...
          <Reveal delay={0.08} className="max-w-xl text-base leading-8 text-white/60 sm:text-lg">
          ...
          {conversationPillars.map((pillar, index) => (
            <Reveal key={pillar.number} delay={index * 0.04}>
              <motion.article
                ...
                className="group grid gap-4 border-b border-white/16 py-8 sm:grid-cols-[4rem_1fr] lg:grid-cols-[5rem_.7fr_1fr] lg:items-center lg:gap-10 lg:py-10"
              >
                <span className="text-[0.66rem] font-extrabold tracking-[0.16em] text-white/42">{pillar.number}</span>
                <h3 className="text-[clamp(2.8rem,5vw,5.2rem)] font-medium leading-none tracking-[-0.055em]">{pillar.title}</h3>
                <p className="max-w-xl text-sm leading-7 text-white/56 sm:col-start-2 sm:text-base lg:col-start-auto lg:justify-self-end lg:leading-8">{pillar.description}</p>
              </motion.article>
            </Reveal>
          ))}
        </div>
