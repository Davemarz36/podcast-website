...
export function WhyWeExist() {
  return (
    <section id="why" aria-labelledby="why-heading" className="section-pad relative overflow-hidden bg-charcoal text-ivory">
      <div className="grain absolute inset-0 opacity-20" />
      <div className="page-shell relative">
        <div className="grid gap-12 lg:grid-cols-[.9fr_1.1fr] lg:gap-24">
          <Reveal>
            <p className="eyebrow text-copper">Why we exist</p>
            <h2 id="why-heading" className="mt-5 max-w-2xl font-display text-[clamp(3.2rem,6.2vw,6.6rem)] leading-[0.88] tracking-[-0.038em]">
          ...
          <Reveal delay={0.08} className="max-w-2xl space-y-5 text-base leading-8 text-white/62 sm:text-lg lg:pt-11">
            ...
            <p className="font-display text-4xl leading-none text-ivory sm:text-5xl">We are interested in the middle.</p>
          ...
          <ol aria-label="From story to courage" className="grid gap-0 sm:grid-cols-4">
            {progression.map((item, index) => (
              <li key={item} className="group flex items-center justify-between border-b border-white/12 py-4 last:border-b-0 sm:border-b-0 sm:border-r sm:px-5 sm:last:border-r-0">
                <span className="font-display text-2xl transition-transform duration-300 group-hover:translate-x-1 sm:text-3xl">{item}</span>
                <span className="text-copper" aria-hidden="true">{index < progression.length - 1 ? "→" : "•"}</span>
              </li>
            ))}
          </ol>
        </Reveal>
